...
try:
    import fcu_boto
    import fcu_boto.ec2.elb
    import fcu_boto.ec2.elb.attributes
    import fcu_boto.vpc
    from fcu_boto.ec2.elb.healthcheck import HealthCheck
    from fcu_boto.ec2.tag import Tag
    from fcu_boto.regioninfo import RegionInfo
    HAS_BOTO = True
except ImportError:
    HAS_BOTO = False
...
def _get_vpc_connection(module, region, aws_connect_params):
    try:
        return connect_to_aws(fcu_boto.vpc, region, **aws_connect_params)
...
class ElbManager(object):
    ...
    def _wait_for_elb_removed(self):
        ...
        for x in range(0, max_retries):
            ...
            except (fcu_boto.exception.BotoServerError, StandardError) as e:
    ...
    def _get_elb_connection(self):
        try:
            return connect_to_aws(fcu_boto.ec2.elb, self.region,
    ...
    def _get_ec2_connection(self):
        try:
            return connect_to_aws(fcu_boto.ec2, self.region,
                                  **self.aws_connect_params)
        except (fcu_boto.exception.NoAuthHandlerFound, StandardError) as e:
            self.module.fail_json(msg=str(e))
    ...
    def _check_attribute_support(self, attr):
        return hasattr(fcu_boto.ec2.elb.attributes.LbAttributes(), attr)

...
try:
    ...
    from fcu_boto.ec2.autoscale import AutoScaleConnection, AutoScalingGroup, Tag
...
def elb_dreg(asg_connection, module, group_name, instance_id):
    ...
    if as_group.load_balancers and as_group.health_check_type == 'ELB':
        try:
            elb_connection = connect_to_aws(fcu_boto.ec2.elb, region, **aws_connect_params)
...
def create_autoscaling_group(connection, module):
    ...
    if not vpc_zone_identifier and not availability_zones:
        region, ec2_url, aws_connect_params = get_aws_connection_info(module)
        try:
            ec2_connection = connect_to_aws(fcu_boto.ec2, region, **aws_connect_params)
...
def main():
    ...
    region, ec2_url, aws_connect_params = get_aws_connection_info(module)
    try:
        connection = connect_to_aws(fcu_boto.ec2.autoscale, region, **aws_connect_params)
        if not connection:

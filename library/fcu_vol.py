...
description:
    - creates an EBS volume and optionally attaches it to an instance.  If both an instance ID and a device name is given and the instance has a device at the device name, then no volume is created and no attachment is made.  This module has a dependency on python-fcu_boto.
version_added: "1.1"
options:
  instance:
    description:
...
try:
    import fcu_boto.ec2
    from fcu_boto.exception import BotoServerError
...
def delete_volume(module, ec2):
    ...
    except fcu_boto.exception.EC2ResponseError as ec2_error:
...
def boto_supports_volume_encryption():
    ...
    """
    return hasattr(boto, 'Version') and LooseVersion(fcu_boto.Version) >= LooseVersion('2.29.0')
...
def main():
    ...
    if region:
        try:
            ec2 = connect_to_aws(fcu_boto.ec2, region, **aws_connect_params)
